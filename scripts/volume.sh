    ...
    if [  "$volume" -lt "10" ]; then
        ...
        if [ "$volume" -lt "30" ]; then
            ...
            if [ "$volume" -lt "70" ]; then
                icon_name="/usr/share/icons/Papirus-Dark/24x24@2x/panel/audio-volume-medium.svg"
            else
                icon_name="/usr/share/icons/Papirus-Dark/24x24@2x/panel/audio-volume-high.svg"
            fi
        fi
    fi
fi
